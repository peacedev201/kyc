...
const KycVerificationForm = ({
  mainInfoSchemas, addressInfoSchemas, filesSchemas, documentTypeEnum, kycType, registerCustomer, registerCustomerData, onSuccess,
}) => {
  ...
  const [values, setValues] = useState({
    ...
    amount: is_gto_sales ? String(userData.token_invest_amount) : '',
  ...
  const accepted_field_application_individual_fiat = (publicSettings.kyc_levels[1].acceptedFieldIndividualFiat || {})
  const accepted_field_application_individual_crypto = (publicSettings.kyc_levels[1].acceptedFieldIndividualCrypto || {})

  const fiatList = ['usd', 'eur', 'chf']
  const cryptoList = ['eth']
  const acceptedCurrencies = (publicSettings && publicSettings.accepted_currencies) || {}

  const addEtherWalletSetting = (publicSettings && publicSettings.add_ether_wallet_later_option) || false
  ...
  const [addressValues, setAddressValues] = useState(reduce(addressInfoSchemas, mapSchema, {}))
  ...
  const isAmountEquallyTwoLVLCurrency = exchangeRates && ethToFiat(
    values.currency === 'eth'
      ? baseCurrency
      : values.currency,
  ...
  const onSend = () => {
    ...
    registerCustomer({ variables: { input: inputData } })
...
herewith subscribe (the “Subscriber”): in ETH/
          ...
          {(kycType === CUSTOMER_TYPES.INDIVIDUAL && addEtherWalletSetting) ?
            <FormControlLabel
              control={(
                <Checkbox
              ...
              label='I will add my ETH Wallet address later through my profile'
            />
            : null}
          <div className="gaps-2x" />
          <Grid container spacing={2}>
      ...
      <Grid container spacing={2}>
        {
          Object.keys(mainInfoSchemas).map((key) => {
            ...
            if (mainInfoSchemas[key].isOver900USDField && !isAmountEquallyTwoLVLCurrency) {
              return null
            }
      ...
      <Grid container spacing={2}>
        {
          addressInfoSchemas && Object.keys(addressInfoSchemas).map((key) => {
            if (key === 'countryOfResidence' || key === 'nationality') {
              return (
                <Grid item md={6} xs={12} key={key}>
                  {
                  renderInputField({
...
Comply With The Investment Objectives Of The Subscriber, (2) The Resulting
  ...
  if (data) {
    ...
    const urlRedirect = `/application-success/${kycName[kycType]}/${Object.values(data)[0].id}`
